...
char book[] = {
#embed "frank.txt" /// Stores the content of the file as an array of chars.
...
void replace_non_printable_chars_with_space() {
  for(size_t i = 0; book[i] != '\0'; ++i) //køre for alle tegn
  {
    if (!isprint((unsigned char)book[i])) //isprint() kommer fra ctype.h (!isprint betyder isprint = false)
    {
      book[i] = ' ';//laver mellemrum der hvor teksten ikke er kan skrives
    }
  }
}
...
void tokenize_and_fill_succs(char *delimiters, char *str) {
  ...
  while (tok != NULL) {
    // register token (token_id will add if needed)
    token_id(tok);
    if (prev != NULL) {
...
{
  if (tokens_size == 0) return 0; //vis der ikke er lavet nogen tokens endnu

  for (size_t attempts = 0; attempts < tokens_size; ++attempts) //prøver at finde en token med det bogstav som starter en sætning, maks er in til der ikke er flere tokens
  {
    size_t candidate = (size_t)rand() % tokens_size; //vælger et tilfældigt tal mellem 0 og tokens_size-1
    if (tokens[candidate] != NULL && isupper((unsigned char)tokens[candidate][0]))
    {
      return candidate;
    }
  }
  return 0; //fallback hvis der ikke er nogen tokens der starter med stort bogstav
}
